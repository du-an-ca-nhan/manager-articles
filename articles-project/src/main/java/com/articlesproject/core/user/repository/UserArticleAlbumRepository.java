...
public interface UserArticleAlbumRepository extends ArticlesAlbumRepository {
    ...
    @Query(value = """
             SELECT ar.id, ar.title, ar.descriptive, ar.browse_date, ca.name As name_category, ar.status,ar.users_id, us.img, us.name,  COUNT(tyms.article_id) AS 'tym', IF((SELECT SUM(IF(ty.article_id IS NULL, 0, 1))  FROM tyms ty
