...
import com.articlesproject.core.user.model.response.UserChartHistoryRequest;
...
public interface UserHistoryRepository extends HistoryRepository {
    ...
    @Query(value = """
            SELECT  us.name, us.id, COUNT(ar.users_id) AS view FROM history hi
            LEFT JOIN articles ar ON ar.id = hi.articles_id
            LEFT JOIN users us ON us.id = ar.users_id
            WHERE hi.users_id = :usersId
            GROUP BY us.name, us.id
            ORDER  BY count(hi.users_id) DESC
            """, nativeQuery = true)
    List<UserChartHistoryRequest> findAllUsersWhoSeeArticleAnyone(@Param("usersId") String usersId);
}
